...
def send_otp_email(receiver_email, otp_code):
  try:
    ...
    if sender_email and sender_password:
      msg = MIMEText(f"Your Maaz Khan Trading Terminal verification OTP is: {otp_code}\n\nDo not share this code with anyone.")
...
def send_otp_sms(receiver_phone, otp_code):
  try:
    ...
    if account_sid and auth_token and from_number:
      ...
      data = urllib.parse.urlencode({"From": from_number, "To": receiver_phone, "Body": f"Your Maaz Khan Trading Terminal OTP is: {otp_code}. Valid for verification."}).encode()
...
# -----------------------------------------------------------------------------
# 4. SAFE REST DATA ENGINE (PREVENTS RATE LIMIT BANS)
# -----------------------------------------------------------------------------
@st.cache_data(ttl=5)
def get_market_ticker_price(symbol="BTCUSDT"):
  ...
  try:
    ...
    pass
  return {"price": 0.0, "change": 0.0, "high": 0.0, "low": 0.0, "volume": 0.0, "quote_volume": 0.0}
...
if not st.session_state["logged_in"]:
  ...
  with col2:
    tab_login, tab_reg, tab_forgot = st.tabs(["🔒 Account Login", "📝 Trader Registration", "🔑 Reset Password"])

    with tab_login:
      with st.form("login_form"):
        st.subheader("Login to Terminal")
        l_user = st.text_input("Username", key="l_user", autocomplete="username")
        l_pass = st.text_input("Password", type="password", key="l_pass", autocomplete="current-password")
        ...
        if submit_login:
          ...
          if str(l_captcha).strip() != str(ans):
            st.error(f"Incorrect CAPTCHA answer. What is {st.session_state['cap_a']} + {st.session_state['cap_b']}?")
    ...
    with tab_reg:
      if st.session_state["reg_step"] == "details":
        ...
        contact_method = st.radio("Verification Method:", ["Phone Number", "Email Address"], horizontal=True)
        
        if contact_method == "Phone Number":
          country_codes = ["+92 (Pakistan)", "+1 (USA)", "+44 (UK)", "+971 (UAE)"]
          selected_cc = st.selectbox("Select Country Code", country_codes)
          country_code = selected_cc.split(" ")[0]
          raw_phone = st.text_input("Mobile Phone Number")
        ...
        if not sent_status:
          st.warning(f"⚠️ {status_msg}\n\n📩 [FALLBACK DISPATCH DISPLAY] Your OTP code is: **{st.session_state['generated_otp']}**")

        user_otp = st.text_input("Enter 6-Digit OTP Code", max_chars=6, key="user_otp")
        col_v1, col_v2 = st.columns(2)
        with col_v1:
          if st.button("Verify OTP & Complete Registration"):
            if user_otp.strip() == st.session_state["generated_otp"]:
              success = add_user(pending["username"], pending["password"], pending["contact_type"], pending["contact_info"])
              if success:
    ...
    with tab_forgot:
        st.info("Follow the same OTP flow as registration to reset.")
  ...
  selected_pair = st.sidebar.selectbox(
      "Select Market Pair:",
      ["BTCUSDT", "ETHUSDT", "SOLUSDT", "BNBUSDT", "XRPUSDT", "DOGEUSDT", "ADAUSDT", "AVAXUSDT", "NEARUSDT", "PEPEUSDT", "SUIUSDT", "LINKUSDT"],
      index=0,
  )
  ...
  c2.metric("24h Change", f"{ticker_data['change']:.2f}%", delta=f"{price_chg_color} 24h")
  ...
  # ⚡ CLEAN NATIVE SCALP METRIC CARDS
  ...
  with tab_chart:
    st.subheader(f"📈 Live WebSocket Charting Engine ({timeframe} Timeframe): {selected_pair}")
  ...
  with tab_signals:
    ...
    if not df_klines.empty and len(df_klines) > 20:
      ...
      with s1:
        ...
        st.markdown(f"**Bias:** {'<span class="badge-long">LONG</span>' if is_bull else '<span class="badge-short">SHORT</span>'}", unsafe_allow_html=True)
      ...
      with s3:
        ...
        lb_val = float(lower_band.iloc[-1]) if not lower_band.empty and not pd.isna(lower_band.iloc[-1]) else curr_p * 0.95
        st.write(f"**Primary Support:** {fmt_p(lb_val)}")
  ...
  with tab_orderbook:
    ...
    st.table(
        pd.DataFrame([
            {"Institution": "BlackRock (IBIT)", "Flow": "+$184.2M", "Bias": "Bullish 🟢"},
            {"Institution": "Fidelity (FBTC)", "Flow": "+$62.5M", "Bias": "Bullish 🟢"},
            {"Institution": "Grayscale (GBTC)", "Flow": "-$38.0M", "Bias": "Outflow 🔴"},
        ])
    )
